#ifndef IK_CONSTRAINT2_MULTICOLLISIONCONSTRAINT_H
#define IK_CONSTRAINT2_MULTICOLLISIONCONSTRAINT_H

#include <ik_constraint2/IKConstraint.h>

namespace ik_constraint2{
  class MultiCollisionConstraint : public IKConstraint
  {
  public:
    ...
    //  fatalDistance: この距離以下で近接している場合、制約をフリーにする. めり込みすぎて離れる方向を正しく計算できない場合に用いる
    //  invert: 干渉しないようにするのではなく、干渉するように制約を加える. ignoreDistanceを大きくせよ. trueだとメッシュの端点で振動しがち
    ...
    const double& fatalDistance() const { return fatalDistance_;}
    double& fatalDistance() { return fatalDistance_;}
    ...
    void copy(std::shared_ptr<MultiCollisionConstraint> ret, const std::map<cnoid::BodyPtr, cnoid::BodyPtr>& modelMap) const;

  protected:
    //A_v, B_vはlocal系. directionは離れる方向が正. distanceはdistancesの最小値
    virtual bool computeDistance(const cnoid::LinkPtr A_link, const cnoid::LinkPtr B_link, double& distance, std::vector<double>& distances, std::vector<cnoid::Vector3>& direction/*B->A*/, std::vector<cnoid::Vector3>& A_v, std::vector<cnoid::Vector3>& B_v)=0;
  ...
  protected:
    ...
    double fatalDistance_ = -0.1;
    bool invert_ = false;

    cnoid::SgLineSetPtr lines_;
    std::vector<cnoid::SgNodePtr> dummyDrawOnObjects_;

    std::vector<cnoid::Vector3> A_currentLocalp_;
    std::vector<cnoid::Vector3> B_currentLocalp_;
    std::vector<cnoid::Vector3> currentDirection_;
    std::vector<double> currentDistances_;
