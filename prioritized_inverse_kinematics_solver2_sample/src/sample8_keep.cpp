...
namespace prioritized_inverse_kinematics_solver2_sample{
  void sample8_keep(){
    ...
    std::shared_ptr<ik_constraint2::PoseKeepCollisionConstraint> constraint = std::make_shared<ik_constraint2::PoseKeepCollisionConstraint>();
    ...
    constraint->A_FACE_C()[0].conservativeResize(constraint->A_FACE_C()[0].rows()+1,constraint->A_FACE_C()[0].cols()+3);
    constraint->A_FACE_dl()[0].conservativeResize(constraint->A_FACE_dl()[0].rows()+1);
    constraint->A_FACE_du()[0].conservativeResize(constraint->A_FACE_du()[0].rows()+1);
    constraint->A_FACE_C()[0].insert(constraint->A_FACE_C()[0].rows()-1,5) = 1;
    constraint->A_FACE_dl()[0][constraint->A_FACE_dl()[0].rows()-1] = -M_PI/2;
    constraint->A_FACE_du()[0][constraint->A_FACE_du()[0].rows()-1] = M_PI/2;

    for(int i=-10;i<=10;i++){
      for(int j=-10;j<=10;j++){
        cnoid::Isometry3 p;
        p.translation() = cnoid::Vector3(0.1*i,0.1*j,0);
        p.linear() = cnoid::Matrix3::Identity();
        constraint->B_POSE().push_back(p);
      }
    }
    ...
    while(true){
      ...
      moveconstraint->B_localpos().linear() = cnoid::Matrix3(cnoid::AngleAxisd(M_PI * std::sin(i / 250.0),cnoid::Vector3::UnitZ()));
