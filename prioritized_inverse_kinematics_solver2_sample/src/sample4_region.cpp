...
namespace prioritized_inverse_kinematics_solver2_sample{
  void sample4_region(){
    ...
    {
      // task: rarm to target. region task
      ...
      constraint->B_link() = nullptr;
      constraint->B_localpos().translation() = cnoid::Vector3(0.7,-0.2,0.8);
      ...
      Eigen::MatrixXd polygon(3,3);
      polygon.col(0) = Eigen::Vector3d(-0.3,0,0);
      polygon.col(1) = Eigen::Vector3d(0.3,0,0);
      polygon.col(2) = Eigen::Vector3d(0,0.3,0);
      constraint->setVertices(polygon);
