  ...
  void sample5_or(){
    ...
    std::vector<std::shared_ptr<ik_constraint2::IKConstraint> > constraints1;
    {
      std::shared_ptr<ik_constraint2::ORConstraint> orconstraint = std::make_shared<ik_constraint2::ORConstraint>();
      {
        ...
        orconstraint->children().push_back(constraint);
      }
      {
        // task: lleg to ground
        std::shared_ptr<ik_constraint2_bullet::BulletCollisionConstraint> constraint = std::make_shared<ik_constraint2_bullet::BulletCollisionConstraint>();
      ...
      constraints1.push_back(orconstraint);
    }
    {
      // task: COM to target
    ...
    std::vector<std::shared_ptr<ik_constraint2::IKConstraint> > constraints2;
    {
      ...
      constraint->B_localpos().translation() = cnoid::Vector3(1.0,-0.2,2.0);
    ...
    }
    {
      ...
      constraint->A_localpos().translation() = cnoid::Vector3(0.0,0.0,-0.02);
      constraint->B_link() = nullptr;
      constraint->B_localpos().translation() = cnoid::Vector3(0.3,0.2,2.0);
      for(size_t i=0;i<3;i++)constraint->weight()[3+i] = 0.0;
      constraints2.push_back(constraint);
    }
